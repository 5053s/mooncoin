...
unsigned int GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (gArgs.GetBoolArg("-testnet", false)){
        return nProofOfWorkLimit.GetCompact(); //testnet build
    }
    ...
    if ((pindexLast->nHeight+1) % params.DifficultyAdjustmentInterval() != 0)
    {
		...
		LogPrintf("params.nPowTargetSpacing  %d \n", params.nPowTargetSpacing);
            ...
            else
            {
                ...
                while (pindex->pprev && pindex->nHeight % params.DifficultyAdjustmentInterval() != 0 && pindex->nBits == nProofOfWorkLimit.GetCompact())
                    pindex = pindex->pprev;
    ...
    if (bnNew > nProofOfWorkLimit)
        ...
        /// debug print
	if(gArgs.GetBoolArg("-debug", false)){
		LogPrintf("GetNextWorkRequired RETARGET Start()\n");
		if (fTestNet) {
	  // testnet build fast
	   LogPrintf("Params().TargetTimespan() = %d    nActualTimespan = %d\n", params.nPowTargetSpacing, nActualTimespan);
		LogPrintf("Before: %08x  %s\n", pindexLast->nBits, bnOld.ToString());
		LogPrintf("After:  %08x  %s\n", nProofOfWorkLimit.GetCompact(), nProofOfWorkLimit.ToString());
       }else {
	    LogPrintf("Params().TargetTimespan() = %d    nActualTimespan = %d\n", params.nPowTargetSpacing, nActualTimespan);
		LogPrintf("Before: %08x  %s\n", pindexLast->nBits, bnOld.ToString());
...
unsigned int KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64_t TargetBlocksSpacingSeconds, uint64_t PastBlocksMin, uint64_t PastBlocksMax, const Consensus::Params& params) {
    ...
    if (BlockLastSolved == NULL || BlockLastSolved->nHeight == 0 || (uint64)BlockLastSolved->nHeight < PastBlocksMin) { return bnProofOfWorkLimit.GetCompact(); }
  	...
  	for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
		...
		else		{ PastDifficultyAverage = ((arith_uint256().SetCompact(BlockReading->nBits) - PastDifficultyAveragePrev) / i) + PastDifficultyAveragePrev; } // arith_uint256 was CBigNum
    ...
    if(gArgs.GetBoolArg("-debug", false)){
		...
		LogPrintf("Before: %08x  %s\n", BlockLastSolved->nBits, arith_uint256().SetCompact(BlockLastSolved->nBits).ToString().c_str()); // arith_uint256 was CBigNum
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    if (pindexLast->nHeight+1 >= 2461218) //DigiShield implementation - thanks to RealSolid & WDC for this code
    {
